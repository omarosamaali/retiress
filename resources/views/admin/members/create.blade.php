...
<div class="col-md-6">
    <div class="mb-3">
        ...
        <select class="form-select" name="council_id" id="councils">
            <option selected value="">اختر المجلس</option>
            @foreach ($councils as $council) {{-- غيّرت اسم المتغير هنا من $councils إلى $council لتجنب التكرار --}}
            <option @selected($council->id == old('council_id')) value="{{ $council->id }}">
                {{ $council->name_ar }}
            </option>
            @endforeach
        </select>
        @error('council_id') {{-- تأكد أن اسم الخطأ يطابق اسم الحقل --}}
        <div class="text-black">{{ $message }}</div>
        @enderror
    </div>
</div>
...
<div class="col-md-6">
    <div class="mb-3">
        ...
        <select class="form-select" name="committee_id" id="committees">
            <option selected value="">اختر اللجنة</option>
            @foreach ($committees as $committee) {{-- غيّرت اسم المتغير هنا من $committees إلى $committee لتجنب التكرار --}}
            <option @selected($committee->id == old('committee_id')) value="{{ $committee->id }}">
                {{ $committee->name_ar }}
            </option>
            @endforeach
        </select>
        @error('committee_id') {{-- تأكد أن اسم الخطأ يطابق اسم الحقل --}}
        <div class="text-black">{{ $message }}</div>
        @enderror
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const councilsSelect = document.getElementById('councils');
        const committeesSelect = document.getElementById('committees');

        function handleSelectChange() {
            if (councilsSelect.value !== '') { // إذا تم اختيار مجلس
                committeesSelect.value = ''; // قم بإعادة تعيين اختيار اللجنة
                committeesSelect.disabled = true; // وعطل قائمة اللجان
            } else {
                committeesSelect.disabled = false; // وإلا، قم بتمكين قائمة اللجان
            }

            if (committeesSelect.value !== '') { // إذا تم اختيار لجنة
                councilsSelect.value = ''; // قم بإعادة تعيين اختيار المجلس
                councilsSelect.disabled = true; // وعطل قائمة المجالس
            } else {
                councilsSelect.disabled = false; // وإلا، قم بتمكين قائمة المجالس
            }
        }

        // استمع لتغييرات كل قائمة
        councilsSelect.addEventListener('change', handleSelectChange);
        committeesSelect.addEventListener('change', handleSelectChange);

        // شغل الدالة عند تحميل الصفحة عشان تحدد الحالة الأولية لو فيه قيمة قديمة
        handleSelectChange();
    });

</script>
@endpush <div class="col-md-6">
