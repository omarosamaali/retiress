...
@section('title', 'تعديل المجله')
@section('page-title', 'تعديل المجله')
...
@section('content')
    <div class="add-section">
        <h5 class="mb-4">
            ...
            تعديل المجله: {{ $magazine->title_ar }}
        </h5>
        ...
        <form action="{{ route('admin.magazines.update', $magazine->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="title_ar" class="form-label font-bold">عنوان المجله (بالعربية)</label>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="status" id="status" required>
                            <option value="1" {{ old('status', $magazine->status) == '1' ? 'selected' : '' }}>فعال
                            </option>
                            <option value="0" {{ old('status', $magazine->status) == '0' ? 'selected' : '' }}>غير فعال
                            </option>
                        </select>
                        @error('status')
                            <div class="text-black">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @if ($magazine->main_image)
                            <img id="current_main_image_preview" src="{{ $magazine->main_image_url }}" alt="الصورة الحالية" class="news-preview mt-2 p-2 current-image-preview">
                ...
                <div class="col-md-12">
                    <div class="mb-3">
                        <label for="description_ar" class="form-label font-bold">وصف المجله (بالعربية)</label>
                ...
                @foreach ($targetLanguages as $code => $name)
                    <div class="col-md-12 mb-3 border rounded-lg p-2">
                        ...
                        <span class="text-right">
                            {{ old('title_' . $code, $magazine->{'title_' . $code}) }}
                        </span>
                        <br>
                        <label for="description_{{ $code }}" class="form-label font-bold">{{ $name }} (الوصف):</label>
                        <span class="text-right">
                            {{ old('description_' . $code, $magazine->{'description_' . $code}) }}
                        </span>
                    </div>
                @endforeach
            </div>
