...
class FeatureController extends Controller
{
    ...
    public function update(Request $request, Feature $feature)  // ✅ صحيح
    {
        $request->validate([
            'title_ar' => 'required|string|max:255',
            'main_image' => 'nullable|image|max:2048',
        ...
        try {
            ...
            if ($request->hasFile('main_image')) {
                if ($feature->main_image) {  // ✅ غيّر من $magazine إلى $feature
            ...
            if ($request->hasFile('sub_images')) {
                if ($feature->sub_image) {  // ✅ غيّر من $magazine إلى $feature
                    $oldSubImages = json_decode($feature->sub_image, true);
            ...
            $feature->update($magazineData);  // ✅ غيّر من $magazine إلى $feature
